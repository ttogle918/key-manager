...
주소당 기본 **시간당 10회**, IP당 **시간당 30회**입니다. 슬라이딩 창이라 각 요청은 발생 후
1시간이 지나면 카운트에서 빠집니다 - 하루 단위로 리셋되는 방식이 아닙니다.
`RELAY_RATE_LIMIT_PER_EMAIL`·`RELAY_RATE_LIMIT_PER_IP`·`RELAY_RATE_LIMIT_WINDOW_SECONDS`
로 조절합니다.

확인 메일 발송이 실패하면(SMTP 오류) 그 요청은 **한도에서 되돌려집니다.** 메일이 한 통도
나가지 않았으니 어뷰징이 아니고, 서버 사정으로 사용자 몫을 깎을 이유가 없습니다.

확인 코드 입력(`POST /sync/confirm`)은 한도를 쓰지 않습니다. 제한 대상은 메일을 발생시키는
`POST /sync/request` 뿐입니다.
