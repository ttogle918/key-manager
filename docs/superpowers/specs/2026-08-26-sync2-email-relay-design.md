...
사용자의 암호문 번들이나 이메일 주소를 저장하지 않는다 — 요청이 끝나면 아무것도 남지 않는다.
...
핵심: 매니저 릴레이는 **평문을 절대 보지 않고**(번들은 어차피 암호문), **아무 것도 영구 저장하지
않는다**(요청 처리 후 메모리에서 삭제). 로그인·계정·RLS 개념 자체가 없다.
...
이 프로젝트의 실제 제출/시연에는 ttogle918이 운영자로서 GCloud Cloud Run에 배포한다(**min-instances=1**
권장 — 스케일-투-제로가 되면 메모리 토큰 저장소가 리셋되어 대기 중인 확인 요청이 유실될 수 있음).
...
- 번들 형식 이상(SYNC-0 검증 실패): `422`(새 검증 로직 없음 — 기존 번들 형식 검사만 통과하면 내용은 안 들여다봄)
