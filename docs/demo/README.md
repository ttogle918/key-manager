...
실제 브라우저에서 앱을 조작하며 캡처한 라이브 녹화본이다(2026-09-04, v0.6 기준). 더미 값으로
마스터 비밀번호 설정 → `.env` 드롭·가져오기 표 → 컬렉션 지정 후 일괄 저장 → 보관함(컬렉션·서비스별
그룹, 노출 등급 배지) → **승인 대기**(SDK 요청 수신 → 허용 → "이미 허용한 디렉토리"가 그 자리에서
1개로 늘어남 → 철회 버튼) → **컬렉션 접근**(경로 직접 입력 + 데스크톱 전용 "찾아보기") →
**이메일로 내보내기**(화면에만 뜨는 6자리 확인 코드)까지 담았다. `demo.gif`(정지 자산)와 달리 실제
UI 상호작용을 그대로 기록한 것이라 기능이 실제로 동작함을 보여주는 용도다.

담긴 값은 전부 더미다. 확인 포인트:

- `DATABASE_URL` 과 `DB_URL` 이 값이 같은데도 **둘 다 남는다**.
- 승인 직후 "이미 허용한 디렉토리"가 0개 → 1개로 바뀐다. 이 녹화 도중 그게 안 되는 걸 발견해
  고쳤다(승인 후 목록을 다시 불러오지 않던 버그).
- 확인 코드는 **메일이 아니라 앱 화면에만** 뜬다.

---

## `.env` 가져오기 워크스루 GIF (`env-import-walkthrough.gif`)

실제 브라우저에서 캡처한 라이브 녹화본이다(2026-09-03). 마스터 비밀번호 설정 → `.env` 드롭 →
가져오기 표(마스킹·"제안" 버튼) → 인라인 편집과 Escape 취소 → 컬렉션 지정 후 일괄 저장 →
보관함(PostgreSQL·미지정 그룹) → 변수명 칸 클릭으로 상세 열기 → 서비스 드롭다운 →
꾹 눌러 그룹 간 드래그 이동까지 담았다.

담긴 값은 전부 더미다. 확인 포인트:

- `DATABASE_URL` 과 `DB_URL` 이 **값이 같은데도 둘 다 남는다** — 프론트가 `.env` 를 직접
  파싱하는 이유다(`/analyze` 는 값 기준으로 중복을 제거한다).
- `MY_CUSTOM_NAME` 이 원본 이름을 지키면서 GitHub 으로 분류된다 — 공식 이름은 "제안"으로만 뜬다.
- 비밀번호가 없는 `DB_HOST` 는 일부러 분류하지 않아 **"미지정"** 으로 남는다.
