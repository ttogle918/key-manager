...
| 4 | 스토어 슬라이스 분할 | 중간 | 13개 섹션이 이미 seam. 한 번에 하지 말 것 (**SDK 구역 1개 완료**) |
| 5 | 스키마 보장을 `user_version` 으로 | 낮음 | 자가 치유 유지가 조건 |
| 6 | MUI 제거(DataGrid 자체 구현) | 중간 | 이득 대비 비용을 먼저 재라 |

> **진행 상황(2026-09-04)**: 첫 조각으로 SDK 접근 관리를 `store/slices/sdkAccess.ts` 로
> 분리했다(상태 7 + 액션 12, 251줄). `keylensStore.ts` 는 1,736 → 1,572줄.
> 슬라이스는 전체 스토어 타입을 가져오지 않고 **필요한 것만 구조적으로 선언**한다
> (`SdkAccessDeps` = `showToast` + `locked` 쓰기) - 순환 참조를 피하면서 의존을 명시한다.
> 검증은 타입·린트만으로는 부족해서, 실제 앱에서 SDK 승인 왕복을 다시 눌러 대기 뱃지가
> 사라지고 "이미 허용한 디렉토리"가 0 → 1 로 늘어나는 것까지 확인했다.
