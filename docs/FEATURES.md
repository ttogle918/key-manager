...
- **컬렉션별 아코디언 그룹**(최근 개편) — 서비스별 그룹에서 컬렉션별 그룹으로 전환. 서비스는
  상단 **로고 태그**(6종, `simple-icons` 벤더링)로 승격돼 컬렉션 횡단 필터 역할을 합니다
- 컬렉션 미지정 시 등록일을 실제 `project` 값으로 자동 저장(빈 버킷 방지, `keylens-env`의
  "미지정 = 전역 키" 규칙과 충돌 방지)
...
- 매니저와 그의 메일 제공자는 서비스명·라벨·컬렉션·메모 같은 **메타데이터**는 볼 수 있지만
...
- **컬렉션 단위 접근범위**: 컬렉션이 지정된 키는 등록된 디렉토리에서만, 미지정("기본") 키는
  전역. 이름이 겹치면 컬렉션 쪽이 우선
...
- **입력 화면**: 스크린샷 드롭/붙여넣기 + URL + 텍스트 + **직접 입력 탭**(키/값 자동분리 + Tab
  자동완성) → 결과 카드(신뢰도·근거·충돌 해소·노출 등급·도움말) → 컬렉션·메모 달아 저장
...
| POST | `/sdk/env` | (SDK 전용) 컬렉션 스코프 값 반환 |
| GET | `/sdk/projects` | 컬렉션 목록 |
