...
- **엔드투엔드 감사(2026-08-29)**: 실제 레포에서 쓰기 전 전 구간을 돌려 점검 → **버그 9건 발견·수정**
  ...
  - [ ] 🧪 패키징된 exe 실행 확인(창 뜸 · "컬렉션 접근" 메뉴 · 승인 뱃지 자동 갱신) — 부분 검증
    - [x] 릴리스 zip 을 새 폴더에 풀어 실행 → 창 뜸, SPA·`/health`·지식베이스 10종·OCR 정상(2026-09-04)
    - [ ] "컬렉션 접근" 메뉴 · 승인 뱃지 자동 갱신 — 사람 눈으로 볼 것(합성 이벤트로 대체 불가)
