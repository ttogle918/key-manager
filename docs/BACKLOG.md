...
### SYNC-2 ⚪ 계정 로그인 기반 서버 동기화 (Supabase, post-MVP — 이번 범위 아님)
- **중요도**: ⚪ Roadmap | **스프린트**: 대회 이후(데스크톱 앱 exe 패키징 마무리 후 착수) | **의존성**: SYNC-0(번들 포맷 재사용), VAULT-1/2 | **사이즈**: M
- **배경**: SYNC-0(수동 파일 이동)·SYNC-1(Google Drive, 로드맵)에 이어 세 번째 멀티 기기 옵션. **로그인하면 서버에 백업, 비회원이면 로컬에만** 남는 옵트인 구조. 로컬 우선 원칙은 그대로 유지 — 로그인 전까지 서버 관련 네트워크 호출이 전혀 없다. SYNC-1(Drive)과는 별개 대안이며, 계정 기반 서버 동기화를 원할 때 이쪽을 선택한다.
- **결정 근거**: 왜 Firebase 대신 Supabase인지, 왜 자동 실시간이 아니라 수동인지 등 각 결정의 이유는 `docs/memo/2026-07-30-sync2-server-sync-decisions.md`에 별도 기록.
- **핵심 설계**
  - [ ] **인증/데이터 분리(제로 널리지 유지)**: Supabase Auth 로그인 ≠ 마스터 비밀번호. 로그인은 "내 데이터가 서버 어디 있는지" 식별용일 뿐, 복호화 열쇠는 항상 로컬 마스터 비밀번호 — 서버는 평문을 절대 보지 않음.
  - [ ] **번들 재사용**: 새 암호화 로직 없음 — 기존 SYNC-0 `.klvault.json` 번들 그대로 사용. 업로드 = `/vault/export` 결과를 Supabase에 저장, 다운로드 = Supabase에서 받은 번들을 그대로 `/vault/import`로 전달.
  - [ ] **저장 스키마**: Supabase Postgres 테이블(사용자당 1행, 최신 번들 전체) 또는 Storage(파일 1개) — 착수 시 확정. 어느 쪽이든 **RLS(행 수준 보안)**로 `auth.uid() = user_id`인 본인 데이터만 read/write 허용(계정 간 상호 열람 차단).
  - [ ] **동기화 트리거(전부 수동)**: (1) 로그인 직후 "서버 백업을 가져올까요?" 확인창, (2) 로그아웃 직전 "로컬 변경사항을 올릴까요?" 확인창, (3) 세션 중 언제든 누르는 수동 "지금 업로드"/"지금 다운로드" 버튼. **자동 실시간 동기화는 하지 않음**(이유는 memo 참고).
  - [ ] **비회원(로컬 전용) 경로 불변**: 로그인 버튼을 누르기 전까지 Supabase 관련 코드는 호출되지 않음 — 기존 로컬 전용 동작 100% 유지.
  - [ ] **리셋 시맨틱**: 로그인 상태에서 금고 리셋 → 로컬 DB + 서버 암호문 행 모두 삭제, **Supabase 계정 자체는 유지**(재가입 없이 새 금고 시작 가능). 비회원 리셋은 기존과 동일(로컬만 삭제).
  - [ ] **신규 의존성**: `@supabase/supabase-js`(MIT) — 착수 시 `THIRD-PARTY-NOTICES.md`·라이선스 스캔 갱신.
- **테스트 체크리스트(착수 시 작성)**
  - [ ] 🧪 로그인 → 서버 백업 있음 → 가져오기 확인창 → 올바른 마스터 비밀번호로 복원
  - [ ] 🧪 로그아웃 → 로컬 변경사항 업로드 확인창 → 서버 반영
  - [ ] 🧪 비회원 상태에서는 Supabase 관련 네트워크 호출이 전혀 발생하지 않음(네트워크 로그로 검증)
  - [ ] 🧪 로그인 상태 리셋 → 로컬+서버 데이터 삭제, 계정 로그인 자체는 유지되는지 확인
  - [ ] 🧪 RLS 정책 — 다른 사용자 계정으로 남의 행 조회/수정 시도 시 거부
- **범위 밖(이번 설계에 안 넣음)**: 실시간 자동 동기화, 여러 기기 간 자동 병합(merge), 소셜 로그인 세부 방식 확정 — 착수 시 결정.
- **착수 시점**: 데스크톱 앱 exe 패키징 마무리 이후로 사용자가 직접 지정.
...
- **핵심 설계**
  - [ ] **실행 전제**: SDK는 자체 암호화·인증 로직을 두지 않는다 — KeyLens 앱(로컬 백엔드 127.0.0.1:8003)이 **켜져있고 잠금 해제된 상태**에서만 동작. 꺼져있거나 잠겨있으면 `load_env()`가 "KeyLens를 켜고 잠금 해제하세요" 같은 명확한 에러를 던진다(조용히 실패·빈 값 반환 금지).
  - [ ] **프로젝트 식별**: 소비 레포 루트에 설정 파일(`.keylens.toml`, `project = "블로그"`)을 둔다. `load_env()`가 `python-dotenv`의 `.env` 탐색과 같은 방식으로 cwd에서 위로 탐색해 찾는다.
  - [ ] **접근 범위(프로젝트 그룹 단위)**: 금고 항목의 기존 "프로젝트" 필드를 정식 그룹 단위로 승격한다.
  ...
  - [ ] **디렉토리 등록(사전 등록 + 승인 프롬프트)**: 설정 화면에서 허용 디렉토리를 미리 등록할 수 있고, **미등록 경로가 처음 요청하면** KeyLens 창에 승인 팝업("`~/repo-a`가 '블로그' 프로젝트 키 요청 — 허용?")이 뜬다. 허용하면 그 경로가 프로젝트에 등록되어 이후 자동 통과(OAuth 동의창과 비슷한 패턴).
  - [ ] **승인 요청 알림(실행 형태별 채널 분리)**: 승인 대기 상태를 놓치지 않도록 실행 형태에 맞는 채널로 알린다.
    - **데스크톱 앱(exe, `desktop/app.py`)**: OS 네이티브 토스트(`plyer`, MIT — permissive) + Windows에서는 표준 라이브러리 `ctypes`로 `FlashWindowEx` 호출해 작업표시줄 아이콘 깜빡임(새 의존성 불필요). macOS(dock 바운스)·Linux(urgency hint)는 후속 확장 — 1차는 Windows 우선.
  ...
  - [ ] **감사 로그**: 기존 VAULT-2 감사 이력에 SDK 경유 조회도 기록 — 어느 경로·프로젝트가 언제 무슨 키를 가져갔는지 남긴다.
