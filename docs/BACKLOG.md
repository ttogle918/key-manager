...
- ✅ **재배포(2026-07-31)**: `v0.1.1` 태그 — 직접 입력 탭(키/값 자동분리 + Tab 자동완성) + 판독불가 화면 뒤로가기 수정 반영해 exe 재빌드.
...
- **테스트 체크리스트**
  ...
  - [x] 🧪 실제 SMTP 자격증명으로 왕복 — ✅ 완료(2026-09-04). 실제 Gmail 로 확인 메일·번들 메일
    두 통 수신 확인. 릴레이 로그로 3단계 전 구간 검증: `POST /sync/request` 202 → 만료 토큰
    `GET /sync/confirm` 410 → 재요청 202 → `GET` 200(**폼만, 발송 없음**) → `POST` 200(발송).
    GET 이 200 을 주고도 아무것도 보내지 않는 것이 핵심이다.
  - [x] **확인 코드로 3단계 발송 전환**(2026-09-04) — 링크를 여는 것(GET)만으로 발송되던 걸
    막았다. Gmail·Outlook ATP 같은 메일 보안 스캐너가 링크를 사용자 대신 미리 열어보면 누르지도
    않았는데 발송되고, 정작 누르면 "만료됐다"가 떴다. 수신 주소를 오타 내면 확인 메일이 낯선
    사람에게 가고 링크 하나가 곧 발송 권한이었다(값은 암호문이지만 서비스명·컬렉션명은 평문).
    이제 GET 은 폼만 보여주고, 앱 화면에만 뜨는 6자리 코드를 POST 로 넣어야 발송된다.
  - [x] **요율 제한 현실화**(2026-09-04) — 주소당 3회/시간은 어뷰저가 아니라 정상 사용자가 먼저
    걸리는 수였다(기기 이전 2회 + 오타 1회 + 코드 만료 재요청이면 끝). 10회/시간으로 올리고
    창(window)도 환경변수로 뺐다. 발송이 SMTP 오류로 실패하면 한도를 되돌린다.
- **범위 밖**: 매니저 릴레이의 영구 저장소, Gmail API/OAuth 경로, 여러 매니저 간 페더레이션.

---

## EPIC-RUNTIME — 런타임 키 주입 SDK (post-MVP — 이번 범위 아님)
...
- **핵심 설계**
  ...
  - [x] **접근 범위(프로젝트 그룹 단위)**: 금고 항목의 기존 "프로젝트" 필드를 정식 그룹 단위로 승격한다.
    ...
    - 같은 환경변수명이 기본·프로젝트 양쪽에 있으면 **프로젝트 쪽이 우선**(override) — 안 겹치면 두 쪽 다 합쳐서 내려준다
  - [x] **디렉토리 등록(사전 등록 + 승인 프롬프트)**(백엔드만 — 프론트 설정 화면·팝업 UI는 별도 플랜): 설정 화면에서 허용 디렉토리를 미리 등록할 수 있고, **미등록 경로가 처음 요청하면** KeyLens 창에 승인 팝업("`~/repo-a`가 '블로그' 프로젝트 키 요청 — 허용?")이 뜬다. 허용하면 그 경로가 프로젝트에 등록되어 이후 자동 통과(OAuth 동의창과 비슷한 패턴).
  - [x] **승인 요청 알림(실행 형태별 채널 분리)**: 승인 대기 상태를 놓치지 않도록 실행 형태에 맞는 채널로 알린다.
    - [x] **데스크톱 앱(exe, `desktop/app.py`)**: OS 네이티브 토스트(`plyer`, MIT — permissive) + Windows에서는 표준 라이브러리 `ctypes`로 `FlashWindowEx` 호출해 작업표시줄 아이콘 깜빡임(새 의존성 불필요). macOS(dock 바운스)·Linux(urgency hint)는 후속 확장 — 1차는 Windows 우선.
...
- **엔드투엔드 감사(2026-08-29)**: 실제 레포에서 쓰기 전 전 구간을 돌려 점검 → **버그 9건 발견·수정**
  ...
  - [x] 🧪 패키징된 exe 실행 확인(창 뜸 · "컬렉션 접근" 메뉴 · 승인 뱃지 자동 갱신) — ✅ 완료(2026-09-04)
    - [x] 릴리스 zip(v0.5.1)을 새 폴더에 풀어 실행 → 창 뜸, SPA·`/health`·지식베이스 10종·OCR 정상
    - [x] 승인 뱃지: 미등록 디렉토리에서 `load_env()` 호출 → 앱에 대기 요청이 **새로고침 없이**
      떴고, 승인하니 사라지면서 `sdk_project_dirs`에 `source='approved'`로 자동 등록됨
    - [x] 승인 후 실제 주입: 컬렉션 5개 변수 전부 전달. `override=True`(기본)는 기존 환경변수를
      금고 값으로 교체하고, `override=False`는 기존 값을 유지하면서 나머지는 주입함
  - [x] **허용 목록 한눈에 보기 + 철회**(2026-09-04) — 승인 대기 화면에 "이미 허용한 디렉토리"
    접힌 섹션을 두고 모든 컬렉션의 허용 경로를 한 번에 보여준다. 목록·해제는 컬렉션 접근 화면에
    이미 있었지만 컬렉션을 하나씩 골라야 보였고, 정작 승인을 누르는 화면에서는 안 보였다 —
    권한을 주는 자리와 회수하는 자리가 떨어져 있으면 회수는 사실상 일어나지 않는다.
  - [x] **네이티브 폴더 찾기**(2026-09-04) — 허용 경로는 정확히 일치해야 매칭되는데 손으로
    타이핑하는 수밖에 없었다. 데스크톱 앱에서 OS 폴더 선택창을 연다. 브라우저에서는 웹 표준이
    절대경로를 주지 않아 불가능하므로 `/desktop/capabilities` 로 먼저 물어보고 버튼을 안 그린다.
...
### RUNTIME-4 🟡 보관함 항목의 서비스 바꾸기(+ 드래그 이동) — ✅ 드롭다운 완료 / 드래그 대기
...
- **하위 할일**
  - [x] `update_meta`에 `service`/`kind`/`label` 추가 + 모델 + 엔드포인트 + 테스트 10건
  - [x] **PATCH 를 부분 수정으로 전환** — 기존엔 전체 교체라 컬렉션만 고쳐도 서비스가 날아갔다.
        `model_fields_set` 으로 "생략"과 "명시적 null" 을 구분한다(JSON 경로까지 테스트로 고정).
  - [x] (service, kind) 조합을 지식베이스로 검증(없는 조합 422) + 라벨은 KB 가 정함
  - [x] 상세보기에 "서비스" 드롭다운(미지정 ↔ 실제 서비스)
  - [x] 드래그 이동을 지름길로 추가(키보드 대체 경로 유지) — ✅ 완료(v0.5.0). 0.6초 꾹 누르면
        들어올려지고, 드래그 중에는 아직 그 컬렉션에 없는 서비스 자리도 나타난다. 드롭다운이
        정확한 경로이고 드래그는 그 위의 지름길 — 스페이스로 집고 화살표로 옮기는 키보드 경로도
        그대로다. `@dnd-kit` 의 delay 제약은 tolerance 가 필수라, 처음 2000ms/8px 로 뒀더니
        손떨림에 취소돼 체감 대기가 훨씬 길었다(600ms/24px 로 조정).
