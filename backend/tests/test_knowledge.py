...
def test_loads_four_services():
    kb = load_knowledge_base()
    assert {s.service for s in kb.services} == {"notion", "kakao", "gcp", "openai"}


def test_credential_counts():
    kb = load_knowledge_base()
    # notion 4 + kakao 4 + gcp 1 + openai 2 = 11
    assert kb.credential_count == 11
...
def test_value_matchers_only_prefix_clear_kinds():
    ...
    assert {"NOTION_API_KEY", "GOOGLE_API_KEY", "OPENAI_API_KEY", "OPENAI_ORG_ID"} <= envs
