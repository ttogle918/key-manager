...
- **보관함 항목의 서비스 바꾸기**: 상세보기의 "서비스" 드롭다운으로 잘못 분류됐거나 미지정으로
  남은 항목을 올바른 서비스로 옮깁니다. 분류기가 모든 걸 맞힐 수는 없으니까요.
  **값은 재암호화 없이 그대로입니다** - 암호문에 묶인 건 변수명(AAD)뿐이라 서비스는 평문
  메타데이터입니다. 변수명 자체를 바꾸는 건 재암호화가 필요해 여전히 별도 작업입니다.
...
- **메타데이터 수정이 전체 교체였던 문제**: `PATCH /vault/entries/{id}` 가 보내지 않은 필드까지
  덮어써서, 컬렉션만 고쳐도 다른 메타데이터가 날아갈 수 있었다. 이제 **보낸 필드만** 수정한다
  (생략과 명시적 `null` 을 구분).
