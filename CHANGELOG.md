...
- **꾹 눌러 끌어 옮기기**: 보관함 항목을 0.6초 누르고 있으면 들어올려져 다른 서비스 그룹으로
  끌어 옮길 수 있습니다. 드래그 중에는 아직 그 컬렉션에 없는 서비스 자리도 함께 나타납니다.
  드롭다운이 정확한 경로이고 드래그는 그 위의 지름길입니다 - 키보드로도 옮길 수 있습니다
  (스페이스로 집고 화살표로 이동).
- **서비스 드롭다운 정렬**: 금고에 이미 항목이 있는 서비스를 위로 올려 "이미 있는 서비스" /
  "새로 지정" 두 그룹으로 나눕니다.
- **연결 실패 화면**: 백엔드에 연결하지 못하면 "금고 만들기" 대신 전용 화면을 띄웁니다.
  금고가 있는지 모르는 상태에서 "새로 만들기"를 권하면, 멀쩡한 금고를 잃은 줄 알고
  새로 만들려 하게 되기 때문입니다.
...
- **메타데이터 수정이 전체 교체였던 문제**: `PATCH /vault/entries/{id}` 가 보내지 않은 필드까지
  ...
  (생략과 명시적 `null` 을 구분).
- **개발 모드에서 앱이 백엔드에 연결하지 못하던 문제**: 백엔드는 `127.0.0.1`(IPv4 전용)에
  바인딩되는데 프론트는 `localhost:8003` 을 불렀다. Windows 에서 `localhost` 는 `::1`(IPv6)로
  먼저 해석되고 **브라우저 fetch 는 IPv4 로 폴백하지 않는다**(curl 은 폴백해서 200 이 나오므로
  터미널로 확인하면 멀쩡해 보인다).
- **끌어 옮기기가 아무 일도 하지 않던 문제**: 드롭 대상을 커서가 아니라 끌리는 요소의 사각형
  겹침으로 골라서, 보관함 행처럼 폭이 넓은 항목은 늘 원래 그룹이 선택돼 "제자리 드롭"이 됐다.

### Security (보안)
- **복호화한 평문을 프론트 스토어에 두지 않는다**: 값을 공개해도 화면에는 앞뒤 4글자만 보이고
  (`sk-p••••••••••••••••hIi4`), 스토어에는 그 문자열만 남는다. 전체 값이 필요한 복사·내보내기는
  백엔드에서 그때그때 새로 받아온다. 평문 체류 시간이 4초에서 0이 됐다.
  (디스크 암호화는 이전과 동일하게 백엔드가 담당한다 - SQLite 에는 암호문만 있다.)
